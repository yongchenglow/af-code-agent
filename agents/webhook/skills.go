...
// RegisterSkills registers all webhook-related skills (deterministic functions)
// Note: Go SDK uses RegisterReasoner for both skills and reasoners
func RegisterSkills(app *agent.Agent) {
	// Main webhook handler (skill: deterministic routing)
	app.RegisterReasoner("handle_webhook", HandleWebhook,
		agent.WithDescription("[SKILL] Processes incoming GitHub webhook events"))

	// PR-specific handler (skill: deterministic workflow orchestration)
	app.RegisterReasoner("handle_pr_event", HandlePREvent,
		agent.WithDescription("[SKILL] Handles pull request events"))

	// Check suite handler (skill: deterministic event processing)
	app.RegisterReasoner("handle_check_suite", HandleCheckSuite,
		agent.WithDescription("[SKILL] Handles CI/CD check suite completion events"))
}
...
func HandlePREvent(ctx context.Context, input map[string]any) (any, error) {
	...
	// Call analyzer skill to analyze the PR
	...
	// Generate fixes with validation (reasoner)
	...
	// Post review comments and apply fixes using the gitops workflow (skill)
...
func handlePushEvent(ctx context.Context, payload map[string]any) (any, error) {
	...
	// Call analyze_push skill to analyze the commits and create/update PR
