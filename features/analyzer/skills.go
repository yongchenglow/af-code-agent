...
// RegisterSkills registers all analyzer skills (deterministic functions)
// Note: Go SDK uses RegisterReasoner for both skills and reasoners
func RegisterSkills(app *agent.Agent) {
	app.RegisterReasoner("analyze_pr", AnalyzePRSkill,
		agent.WithDescription("[SKILL] Analyzes PR files and extracts code metrics"))

	app.RegisterReasoner("analyze_push", AnalyzePushSkill,
		agent.WithDescription("[SKILL] Analyzes push commits and creates/updates PR with fixes"))

	app.RegisterReasoner("fetch_pr_files", FetchPRFilesSkill,
		agent.WithDescription("[SKILL] Fetches changed files from a pull request"))
}

// AnalyzePRSkill orchestrates the complete PR analysis
func AnalyzePRSkill(ctx context.Context, input map[string]any) (any, error) {
...
// AnalyzePushSkill analyzes push commits and triggers PR workflow
func AnalyzePushSkill(ctx context.Context, input map[string]any) (any, error) {
	repo, ok := input["repo"].(string)
	if !ok {
		return nil, fmt.Errorf("repo is required")
	}
...
// FetchPRFilesSkill fetches changed files from a PR
func FetchPRFilesSkill(ctx context.Context, input map[string]any) (any, error) {
	repo, ok := input["repo"].(string)
	if !ok {
		return nil, fmt.Errorf("repo is required")
	}
